                ...
                <form action="" method="post" class="needs-validation" id="servisForm" novalidate>
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" id="no-plat" class="form-control" name="no_plat" required autofocus placeholder="contoh: T 1234 ABC" value="<?= $transaksi->customer->no_plat ?>">
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" id="model-kendaraan" class="form-control" name="model_kendaraan" required autofocus placeholder="contoh: Yamaha Mio" value="<?= $transaksi->customer->model_kendaraan ?>">
                            <div class="invalid-feedback">
                    ...
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" id="nama-pemilik" class="form-control" name="nama_pemilik" value="<?= $transaksi->customer->nama_pemilik ?>">
                        </div>
                        <div class="form-group col-md-6">
                            <label for="no-telp" class="form-label">No Telepon</label>
                            <input type="text" id="no-telp" class="form-control" name="no_telp" value="<?= $transaksi->customer->no_telp ?>">
                        </div>
                    </div>
                </form>
        ...
        <div class="d-flex flex-row gap-3 px-2 justify-content-center">
            <div class="card col-6">
                ...
                <div class="card-footer d-none" id="simpanTransaksi">
                    <button class="btn btn-primary" id="simpan"><i class="fa-solid fa-save"></i> Simpan</button>
            ...
            <form action="#" method="post" id="formTambahItemKeranjang">
                <div class="modal-body">
                    ...
                    <div class="d-none" id="form-input">
                        ...
                        <input type="hidden" name="id_servis" id="id_servis" readonly required>
...
<script>
    ...
    $(function() {
        ...
        $(document).on('submit', '#formTambahItemKeranjang', function(e) {
            ...
            formData.append('id_servis', $('#id_servis').val());
            formData.append('tipe', tipe);
            $.ajax({
                type: 'post',
                url: '<?= base_url('riwayatTransaksi/tambahitemkeranjang') ?>',
                data: formData,
                processData: false,
                contentType: false,
                cache: false,
                ...
                dataTableBarang.ajax.reload()
                $('#formTambahItemKeranjang').trigger('reset')
            }).fail(function(err) {
                console.error(err);
            })
        })
        ...
        $('#hapus-keranjang').click(function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'get',
                        url: '<?= base_url('riwayatTransaksi/hapuskeranjang') ?>',
                    }).done(function(res) {
        ...
        $('#hapus-keranjang').click(function(e) {
            ...
            Swal.fire({
                title: 'Peringatan',
                text: 'Anda yakin untuk mengkosongkan keranjang ini? Transaksi akan hilang bila keranjang di kosongkan!',
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    window.location = '<?= base_url('riwayatTransaksi/hapustransaksi/') ?>' + kode_transaksi;
                }
            })
        });
        ...
        $(document).on('click', '#delete-item', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'get',
                        url: '<?= base_url('riwayatTransaksi/hapusitemkeranjang/') ?>' + id,
                    }).done(function(res) {
            ...
            if (form[0].checkValidity() === false) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'post',
                            url: '<?= base_url('riwayatTransaksi/updatedatatransaksi') ?>',
                            data: {
                            ...
                            dataTableKeranjang.ajax.reload()
                            dataTableBarang.ajax.reload()
                            window.location = '<?= base_url('riwayatTransaksi') ?>'
                        }).fail((err) => {
                            console.error(err);
                        })
                    }
        ...
        $('#simpanCetak').click(function(e) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: 'post',
                        url: '<?= base_url('riwayatTransaksi/updatestatustransaksi') ?>',
                        data: {
                            'kode_transaksi': $('#kode-transaksi').val(),
